...
export const FilterBar = ({ data, onFiltered }) => {
  ...
  // Filter logic separated from data dependency — only re-runs when filters or data change
  const filtered = useMemo(() => {
    ...
    if (dateFrom) r = r.filter(t => Utils.normDate(t.date) >= dateFrom);
    if (dateTo)   r = r.filter(t => Utils.normDate(t.date) <= dateTo);
