...
export const usePassbookReader = () => {
  ...
  const [materialityThreshold, setMaterialityThreshold] = useState(() => parseInt(sessionStorage.getItem('materiality_threshold') || '0', 10));

  const saveApiKey = (key) => { setApiKey(key); sessionStorage.setItem('gemini_api_key', key); };
  const saveMaterialityThreshold = (val) => { setMaterialityThreshold(val); sessionStorage.setItem('materiality_threshold', String(val)); };
  ...
  const updateData = useCallback((globalIndex, field, value) => {
    if (field === 'accountCode') {
      const row = data.find(r => r.globalIndex === globalIndex);
      if (row?.description) AccountPatterns.learn(row.description, value);
    }
    const next = data.map(r => r.globalIndex === globalIndex ? { ...r, [field]: value } : r);
    const needsValidation = field === 'withdrawal' || field === 'deposit' || field === 'balance' || field === 'bankName' || field === 'date';
    setData(needsValidation ? Validators.runAll(next, materialityThreshold) : next);
  }, [data, setData, materialityThreshold]);

  const addRowAfter = useCallback((globalIndex) => {
    const idx = data.findIndex(r => r.globalIndex === globalIndex);
    const ref = data[idx];
    const newRow = {
    ...
    const next = [...data]; next.splice(idx + 1, 0, newRow);
    setData(Validators.runAll(next, materialityThreshold));
  }, [data, setData, materialityThreshold]);

  const deleteRow = useCallback((globalIndex) => {
    setData(Validators.runAll(data.filter(r => r.globalIndex !== globalIndex), materialityThreshold));
  }, [data, setData, materialityThreshold]);

  const processFile = useCallback(async (selectedFile, isAppend = false, accountAlias = null) => {
    if (!apiKey.trim()) return setError('APIキーを保存してください');
    ...
    try {
      ...
      const pageOffset = isAppend ? pageImages.length : 0;
      let newTxs = [];
      ...
      if (selectedFile.type === 'application/pdf') {
        ...
        setPdfProgress({ current: 1, total: 1, text: '画像読み込み済み' });
      ...
      const combined = [...(isAppend ? data : []), ...newTxs];
      const validated = NormalizationRules.applyAll(
      ...
      if (cacheKey) {
        await Utils.cache.set(cacheKey, { data: validated });
      }
  ...
  }, [apiKey, data, pageImages, materialityThreshold]);
  ...
  const applyNormalization = useCallback(() => {
    ...
    dispatch({ type: 'UPDATE', payload: NormalizationRules.applyAll(data) });
  }, [data]);
